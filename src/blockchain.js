...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                const newBlockHeight = self.height + 1;
                block.height = newBlockHeight;
                block.time = this.getCurrentTime;

                if (newBlockHeight > 0) {
                    const previousBlock = await this.getBlockByHeight(self.height);
                ...
                console.error('Error occur while adding a block into the blockchain');
                throw error;
            }
        });
    }
    ...
    requestMessageOwnershipVerification(address) {
        return new Promise((resolve) => {
            resolve(`${address}:${this.getCurrentTime()}:starRegistry`);
        });
    }
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (!verifyMessage) reject(new Error('verification failed'));

            try {
                const block = new BlockClass.Block({address, message, signature, star});
                const newBlock = await this._addBlock(block);
                resolve(newBlock);
            } catch(error) {
                reject(error);
            }
    ...
    validateChain() {
        ...
        let errorLog = [];
        return new Promise(async (resolve, reject) => {
            this.chain.map(async (block, index) => {
                ...
                if(index !== 0 && this.chain[index-1].hash !== block.previousBlockHash) {
                    errorLog.push({
                        message: "Previous does not match",
                        block: block
                    })
                }
            })
